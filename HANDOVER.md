...
タブバー（`#file-tabs`）は `.viewer-header` の**中**に置いてあり、
`戻る + タブ + 操作ボタン` が 1 行に並ぶ。

- ファイル名は**タブにだけ**出す。`#viewer-title` は `hidden` で DOM に残してあるだけ
  （複数箇所の JS が `textContent` を入れているため要素は消せない）
- `.file-tabs` は `flex:1; min-width:0`。タブが増えたら中で横スクロールする
- **699px 以下ではタブに 1 行目を丸ごと使わせ、ボタンを 2 行目に送る**。
  1 行に押し込むとタブが 100px 程度まで潰れてファイル名が読めなくなる
