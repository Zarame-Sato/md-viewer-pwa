...
列数は `repeat(auto-fill, minmax(min(170px, 46%), 1fr))` で画面幅に追従する。

- `min()` で下限を掛けているのは、単純な `minmax(170px, 1fr)` だと iPhone SE 幅で
  **1 列に落ちて巨大タイルになる**ため。狭いときは必ず 2 列を保つ
- サムネ枠は `width:100%` + **`aspect-ratio: 3/4`（縦長）**。固定サイズにすると
  iPad の広い幅でタイルだけ大きくなり、中のサムネが小さく浮いてしまう。
  縦長にしているのは中身が PDF や書類中心（A4 は 1:1.41 の縦長）だから。
  横長の枠だと `object-fit: contain` で縮み、左右が余白だらけになる
